...
async function UserCredentails() {
        ...
        <div className="flex flex-col gap-4 items-center justify-center">
          ...
          <div className="flex flex-col gap-1 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Click the button below to create your first credentail
            </p>
          </div>
  ...
  return (
    <div className="flex gap-2 flex-wrap">
      {credentials.map((creds) => {
        const createdAt = formatDistanceToNow(creds.createdAt, {
          addSuffix: true
        });
        return (
          <Card key={creds.id} className="w-full p-4 flex justify-between">
            ...
            <DeleteCredentialDialog name={creds.name} />
          </Card>
        );
      })}
    </div>
